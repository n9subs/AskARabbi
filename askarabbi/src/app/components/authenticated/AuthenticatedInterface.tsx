...
const ScrollWriterLoader = dynamic(() => import('../ScrollWriterLoader'), { 
  ssr: false, 
  loading: () => <div style={{height: '220px', display: 'flex', alignItems: 'center', justifyContent: 'center'}}>טוען אנימציה...</div>
});

export default function AuthenticatedInterface() {
...
בנוסף להקפות הנערכות בליל שמחת תורה וביומו בבית הכנסת, התפשט גם המנהג של "הקפות שניות" הנערכות במוצאי שמחת תורה (בחוץ לארץ במוצאי שמיני עצרת שהוא גם שמחת תורה), כהמשך לשמחת החג.`,
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (isSubmissionDisabledByTour) return;
      ...
      {showOnboarding && 
        <OnboardingTour
          onComplete={() => {
            ...
            setElementToElevateForTour(null);
            setIsSubmissionDisabledByTour(false);
          }}
          onStepChange={(status) => {
